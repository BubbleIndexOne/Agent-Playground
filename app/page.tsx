...
function BasicModelSettings({ providers, providerId, modelId, apiKey, setProviderId, setModelId, setApiKey, onClose }: { providers: Provider[]; providerId: string; modelId: string; apiKey: string; setProviderId: (v: string) => void; setModelId: (v: string) => void; setApiKey: (v: string) => void; onClose: () => void }) {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="model-settings-title" onMouseDown={e => { if (e.target === e.currentTarget) onClose() }}>
    <div className="w-full max-w-[440px] rounded-2xl border border-border bg-card p-6 shadow-2xl shadow-black/30">
      ...
      <form onSubmit={e => { e.preventDefault(); save(); }} autoComplete="off" className="mt-7 flex flex-col gap-5">
        <input type="text" name="prevent_autofill" className="hidden" tabIndex={-1} aria-hidden="true" autoComplete="off" />
        <input type="password" name="prevent_autofill_pwd" className="hidden" tabIndex={-1} aria-hidden="true" autoComplete="off" />
        ...
        <div className="flex flex-col gap-2"><label htmlFor="settings-key" className="text-[12px] font-medium">API key</label>
          <div className="relative"><LockKeyhole className="pointer-events-none absolute left-3 top-3 size-4 text-muted-foreground" /><input id="settings-key" name="model_api_secret_key" type={visible ? 'text' : 'password'} value={draftKey} onChange={e => setDraftKey(e.target.value)} placeholder="sk-ant-..." autoComplete="new-password" data-lpignore="true" data-1p-ignore="true" className="h-10 w-full rounded-lg border border-border bg-background px-10 pr-10 font-mono text-[12px] text-foreground outline-none placeholder:text-muted-foreground focus:border-primary focus:ring-2 focus:ring-primary/20" /><button type="button" aria-label={visible ? 'Hide API key' : 'Show API key'} onClick={() => setVisible(!visible)} className="absolute right-2 top-2 flex size-6 items-center justify-center rounded text-muted-foreground hover:text-foreground">{visible ? <EyeOff className="size-4" /> : <Eye className="size-4" />}</button></div><p className="text-[11px] leading-5 text-muted-foreground">Stored only for this browser session — cleared when you close this tab. Never sent anywhere except directly to the model provider.</p>
        </div>
        <div className="mt-2 flex items-center justify-between"><button type="button" onClick={clear} className="rounded-md px-1 py-2 text-[12px] text-muted-foreground transition-colors hover:text-destructive">Clear key</button><button type="submit" className="glow-hover rounded-lg bg-primary px-5 py-2.5 text-[12px] font-semibold text-primary-foreground transition-opacity hover:opacity-90">Save</button></div>
      </form>
    </div>
  </div>
}
  ...
  return <div className="fixed inset-0 z-50 flex justify-end bg-background/80 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="model-settings-title" onMouseDown={e => { if (e.target === e.currentTarget) onClose() }}>
    <div className="w-full max-w-[400px] h-full bg-card border-l border-border shadow-2xl flex flex-col animate-in slide-in-from-right duration-200">
      ...
      <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-5">
        {activeTab === 'model' ? (
          <form onSubmit={e => { e.preventDefault(); save(); }} autoComplete="off" className="flex flex-col gap-5">
            <input type="text" name="prevent_autofill_adv" className="hidden" tabIndex={-1} aria-hidden="true" autoComplete="off" />
            <input type="password" name="prevent_autofill_adv_pwd" className="hidden" tabIndex={-1} aria-hidden="true" autoComplete="off" />
            <div className="flex flex-col gap-2"><label htmlFor="adv-provider" className="text-[12px] font-medium">Model provider</label>
              <div className="relative"><select id="adv-provider" value={draftProvider} onChange={(e) => { setDraftProvider(e.target.value); const p = providers.find(p => p.id === e.target.value); if(p) setDraftModel(p.models[0].id) }} className="h-10 w-full appearance-none rounded-lg border border-border bg-background px-3 text-[12px] text-foreground outline-none focus:border-primary focus:ring-2 focus:ring-primary/20">
                {providers.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
              </select><ChevronDown className="pointer-events-none absolute right-3 top-3 size-4 text-muted-foreground" /></div>
            </div>
            <div className="flex flex-col gap-2"><label htmlFor="adv-model" className="text-[12px] font-medium">Model</label>
              <div className="relative"><select id="adv-model" value={draftModel} onChange={(e) => setDraftModel(e.target.value)} className="h-10 w-full appearance-none rounded-lg border border-border bg-background px-3 text-[12px] text-foreground outline-none focus:border-primary focus:ring-2 focus:ring-primary/20">
                {currentProvider.models.map(m => <option key={m.id} value={m.id}>{m.name}</option>)}
              </select><ChevronDown className="pointer-events-none absolute right-3 top-3 size-4 text-muted-foreground" /></div>
            </div>
            <div className="flex flex-col gap-2"><label htmlFor="adv-settings-key" className="text-[12px] font-medium">API key</label>
              <div className="relative"><LockKeyhole className="pointer-events-none absolute left-3 top-3 size-4 text-muted-foreground" /><input id="adv-settings-key" name="model_api_secret_key" type={visible ? 'text' : 'password'} value={draftKey} onChange={e => setDraftKey(e.target.value)} placeholder="sk-ant-..." autoComplete="new-password" data-lpignore="true" data-1p-ignore="true" className="h-10 w-full rounded-lg border border-border bg-background px-10 pr-10 font-mono text-[12px] text-foreground outline-none placeholder:text-muted-foreground focus:border-primary focus:ring-2 focus:ring-primary/20" /><button type="button" aria-label={visible ? 'Hide API key' : 'Show API key'} onClick={() => setVisible(!visible)} className="absolute right-2 top-2 flex size-6 items-center justify-center rounded text-muted-foreground hover:text-foreground">{visible ? <EyeOff className="size-4" /> : <Eye className="size-4" />}</button></div><p className="text-[11px] leading-5 text-muted-foreground">Stored only for this browser session.</p>
            </div>
          </form>
        ) : (
          <>
...
export default function Page() {
  ...
  return <div className="flex min-h-screen bg-background text-foreground" onKeyDown={e => { if (e.key === 'Enter' && (e.ctrlKey || e.metaKey)) { e.preventDefault(); run(); } }}><Sidebar active={active} onSelect={setActive} />
    {active === 'Playground' && (
      <main className="flex min-w-0 flex-1 flex-col"><header className="flex h-[68px] items-center justify-between border-b border-border px-5 md:px-8"><div className="flex items-center gap-3 pl-10 md:pl-0"><div className="hidden size-7 items-center justify-center rounded-md bg-accent text-muted-foreground sm:flex"><PanelLeft className="size-3.5" /></div><div><h1 className="text-[14px] font-semibold tracking-[-0.01em]">Playground</h1><p className="hidden text-[11px] text-muted-foreground sm:block">A calm space to build and test</p></div></div><div className="flex items-center gap-2"><StatusPill icon={Sparkles} label="Model" value={currentModelName} onClick={() => setBasicSettingsOpen(true)} /></div></header>
